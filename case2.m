...
s=c*dt/dx;
u(2,2:lx)=s^2/2*u(1,3:lx+1)+u(1,2:lx)*(1-s^2)+s^2/2*u(1,1:lx-1);  u(2,end)=u(2,end-2);
% u(1,:)=u(3,:);
...
figure;surf(x,t,U);colormap jet;shading interp
% k=.5:.5:20; figure; hold on;
% for i=1:length(k)
% %     k=.5:.5:10;
%     plot(x,U((find(t==k(i))+1),:));
% end
